...
class BaseTable(Base.Composite):
    ...
    def mangle_columns(self, columns, output_options):
        if (   'printable_version' in output_options
            and self.functions):
            columns = Webwidgets.Utils.OrderedDict((name, value) for (name, value) in columns.iteritems()
    ...
    def get_mangled_rows(self, output_options):
        if 'printable_version' in output_options:
            rows = ChildNodeRows(self, self.get_all_rows())
        else:
            rows = self.get_rows()
...
class Table(BaseTable, Base.ActionInput):
    ...
    def mangle_row(self, row, output_options):
        if 'printable_version' in output_options or not self.functions:
            return row
